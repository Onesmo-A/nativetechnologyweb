...
export default function AdminProcessPage() {
  ...
  const canSubmit = useMemo(() => title.trim().length >= 2 && description.trim().length >= 5, [title, description]);
  ...
  const load = async () => {
    ...
    const data = (await res.json()) as { items: ProcessStep[] };
    setItems(data.items);
  };
  ...
  const create = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/admin/process", {
      ...
      body: JSON.stringify({ title, description, order, active }),
    });
    if (!res.ok) {
  ...
  return (
    <div className="space-y-10">
      <div>
        <h1 className="mb-2 text-3xl font-extrabold">Work Process</h1>
        <p className="text-body-color dark:text-body-color-dark">
          Manage steps and ordering.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-stroke bg-white p-6 shadow-one dark:border-white/10 dark:bg-dark">
        <div className="mb-4 flex items-center justify-between">
          <div className="text-lg font-bold">All Steps</div>
          <button
            type="button"
            onClick={() => void load()}
